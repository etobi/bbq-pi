...
var updateProbe = function (probe) {
	...
	var updateDisplay = function(timestamp, probe, value) {
		displayStorage.temp[probe.channel] = value.toFixed(2);
		...
		etobiShield.display.writeLine(6, displayStorage.temp[1] + '      ' + displayStorage.temp[2]);
		etobiShield.display.writeLine(7, displayStorage.temp[3] + '      ' + displayStorage.temp[4]);
	};
	...
	etobiShield.readAdcTemp(probe, function (channel, temp) {
		if (temp) {
			...
			updateDisplay(timestamp, probe, temp);
		} else {
			// TODO error handling
		}
	});
