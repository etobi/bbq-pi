...
var initializeHttpApp = function () {
	...
	io.on('connection', function (socket) {
		...
		socket.on('probes-minmax-update', function (probe) {
			config.probes[probe.channel].min = probe.min;
			config.probes[probe.channel].max = probe.max;
		});
...
var sendSlackMessage = function (message) {
	if (config.slack.webhook) {
		...
		request(options, function (error, response, body) {
			if (error) {
				console.log('Error! (sendSlackMessage/request)', error);
			}
		});
	}
};
...
var updateProbe = function (probe) {
	...
	etobiShield.readAdcTemp(probe, function (channel, temp) {
		if (temp) {
			...
			if (lastAlert == null || timestamp > muteAlertTill) {
				if (temp
					&& probe.min
					&& parseFloat(temp) < parseFloat(Number(probe.min).toFixed(1))) {
					muteAlertTill = Date.now() + (2 * 60 * 1000);
					sendSlackMessage('Achtung! Channel #' + probe.channel + ': ' + temp + ' < ' + Number(probe.min).toFixed(1));
				}
				if (temp
					&& probe.max
					&& parseFloat(temp) > parseFloat(Number(probe.max).toFixed(1))) {
					muteAlertTill = Date.now() + (2 * 60 * 1000);
					sendSlackMessage('Achtung! Channel #' + probe.channel + ': ' + temp + ' > ' + Number(probe.max).toFixed(1));
				}
			}
		} else {
			// TODO error handling
